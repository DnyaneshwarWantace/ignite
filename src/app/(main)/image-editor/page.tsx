...
export default function ImageEditorPage() {
  ...
  const loadProjects = async () => {
    try {
      ...
      const response = await fetch(withBasePath('/api/editor/image/projects'), {
  ...
  const createProject = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewProjectName('');
        router.push(`/image-editor/editor/${data.project.id}`);
      }
    } catch (error) {
  ...
  const deleteProject = async (projectId: string) => {
    try {
      const response = await fetch(withBasePath(`/api/editor/image/projects/${projectId}`), {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  const duplicateProject = async (projectId: string) => {
    try {
      const response = await fetch(withBasePath(`/api/editor/image/projects/${projectId}/duplicate`), {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const renameProject = async () => {
    ...
    try {
      const response = await fetch(withBasePath(`/api/editor/image/projects/${selectedProject.id}`), {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleOpenProject = (projectId: string) => {
    ...
    router.push(`/image-editor/editor/${projectId}`);
  };
  ...
  const platformConfigs = [
    ...
    { id: 'instagram-reel', name: 'Instagram Story', aspectRatio: '9:16' },
    { id: 'youtube-landscape', name: 'YouTube Thumbnail', aspectRatio: '16:9' },
    { id: 'facebook-feed', name: 'Facebook Post', aspectRatio: '1.91:1' },
      ...
      <div className="w-full">
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2 text-black">Image Projects</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <h2 className="text-2xl font-bold mb-4 text-black">No image projects yet</h2>
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Start creating your first image project. Design graphics, edit photos, and export professional images.
            </p>
            <Button
              onClick={() => setShowCreateModal(true)}
              size="lg"
            >
              <Plus className="w-4 h-4 mr-2" />
              Create Your First Image Project
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-6">
            ...
            {filteredProjects.map((project) => (
              <Card key={project.id} className="bg-white border-gray-200 hover:bg-purple-50 hover:border-purple-200 transition-colors shadow-sm">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="bg-white border-gray-200 shadow-lg">
                        <DropdownMenuItem
                          ...
                          className="text-black hover:bg-purple-50"
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold mb-4 text-black">Create New Image Project</h2>
