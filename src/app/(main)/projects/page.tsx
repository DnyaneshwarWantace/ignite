...
export default function ProjectsPage() {
  ...
  const [editorType, setEditorType] = useState<'video' | 'image'>('video');
  ...
  const loadProjects = async () => {
    try {
      ...
      const apiPath = editorType === 'video' 
        ? '/api/editor/video/projects'
        : '/api/editor/image/projects';
  ...
  const deleteProject = async (projectId: string) => {
    try {
      const apiPath = editorType === 'video' 
        ? `/api/editor/video/projects/${projectId}`
        : `/api/editor/image/projects/${projectId}`;
  ...
  const duplicateProject = async (projectId: string) => {
    try {
      const apiPath = editorType === 'video' 
        ? `/api/editor/video/projects/${projectId}/duplicate`
        : `/api/editor/image/projects/${projectId}/duplicate`;
  ...
  const renameProject = async () => {
    ...
    try {
      const apiPath = editorType === 'video' 
        ? `/api/editor/video/projects/${selectedProject.id}`
        : `/api/editor/image/projects/${selectedProject.id}`;
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="flex gap-2 border rounded-lg p-1 bg-gray-50">
              <Button
                variant={editorType === 'video' ? 'default' : 'ghost'}
                size="sm"
                onClick={() => setEditorType('video')}
                className={editorType === 'video' ? '' : 'text-gray-600'}
              >
                Video
              </Button>
              <Button
                variant={editorType === 'image' ? 'default' : 'ghost'}
                size="sm"
                onClick={() => setEditorType('image')}
                className={editorType === 'image' ? '' : 'text-gray-600'}
              >
                Image
              </Button>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Start creating your first {editorType} project. Import media, edit, and export professional content.
            </p>
            <Button 
              onClick={() => setShowCreateModal(true)}
              size="lg"
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold mb-4 text-black">Create New {editorType === 'video' ? 'Video' : 'Image'} Project</h2>
