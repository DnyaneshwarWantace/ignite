...
export const POST = authMiddleware(
  async (request: NextRequest, context: any, user: User) => {
    try {
      ...
      if (!imageUrl) {
        return createError({ message: "Image URL is required" });
      }

      if (!imageUrl.startsWith('https://')) {
        return createError({ message: "Invalid image URL format" });
      }
      ...
      const analysis = await callLLM(user.id, [
        {
          role: "system",
      ...
      return createResponse({
        ...
        payload: { analysis, analysisType, imageUrl }
      });
