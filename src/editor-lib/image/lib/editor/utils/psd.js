  ...
  function attrTransform(childrens) {
    ...
    return childrensFilter.map(async (item) => {
      ...
      const commonAttr = baseUtils([
        'left',
        'top',
        'width', // Remove the +50 offset that causes sizing issues
        'height',
        'name',
        ['visible', () => true],
...
function getClipPath(psdFile) {
  ...
  const workspase = {
    type: 'rect',
    version: '5.3.0',
    id: 'workspace', // Add workspace ID so WorkspacePlugin recognizes it
    originX: 'left',
    originY: 'top',
    left: 0,
    top: 0,
    ...
    lockMovementX: true, // Lock all movements
    lockMovementY: true,
    lockRotation: true,
    lockScalingX: true,
    lockScalingY: true,
